...
public class Crypt {
  ...
  public static String encrypt(final String name, final String password) {
    return ARGON2.hash(
        ARGON2_ITERATIONS, ARGON2_MEMORY, ARGON2_PARALLELISM, password.toCharArray());
  }

  /**
   * Verify a password against a stored hash. Supports both Argon2id hashes and legacy SHA-512
   * hashes. Returns true if the password matches.
   */
